...
def qr(request):
  ...
  if email.strip(): vcardgen.add_vcard_email(vcard, email)
  if phone.strip(): vcardgen.add_vcard_phone(vcard, phone)
  if company.strip() or position.strip(): vcardgen.add_vcard_company(vcard, company, position)
  if street.strip() or city.strip() or region.strip() or postal_code.strip() or country.strip(): vcardgen.add_vcard_address(vcard, street, city, region, postal_code, country)
  if website.strip(): vcardgen.add_vcard_website(vcard, website)
